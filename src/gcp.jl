...
"""
function getgeotransform!(dataset::Ptr{GDAL.GDALDatasetH}, buffer::Vector{Cdouble})
    @assert length(buffer) == 6
    result = GDAL.getgeotransform(dataset, pointer(buffer))
    (result == GDAL.CE_Failure) && error("Failed to get geotransform from raster")
    buffer
end

getgeotransform(dataset::Ptr{GDAL.GDALDatasetH}) =
    getgeotransform!(dataset, Array(Cdouble, 6))

"Set the affine transformation coefficients."
function setgeotransform!(dataset::Ptr{GDAL.GDALDatasetH}, transform::Vector{Cdouble})
    @assert length(buffer) == 6
...
ngcp(dataset::Ptr{GDAL.GDALDatasetH}) = GDAL.getgcpcount(dataset)
...
function setprojection!(dataset::Ptr{GDAL.GDALDatasetH}, projstring::ASCIIString)
    result = GDAL.setprojection(dataset, pointer(projstring))
