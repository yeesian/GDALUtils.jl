...
raster = GU.open("pyrasterio/RGB.byte.tif")
...
raster = GU.create("pyrasterio/example.tif",
                   500, # width
                   300, # height
                   1, # number of bands
                   UInt8, # DataType
...
src = GU.open("pyrasterio/RGB.byte.tif")
rgb = GU.fetch(src, Cint[1,2,3]) # fetch bands 1 - 3
dst = GU.create("pyrasterio/example2.tif",
                500, # width
                300, # height
                3, # number of bands
                UInt8, # DataType
...
rgb = GU.fetch(src, Cint[1,2,3],
               350:410,
               350:450)
dst = GU.create("pyrasterio/example3.tif",
                500, # width
                300, # height
                3, # number of bands
                eltype(rgb), # DataType
